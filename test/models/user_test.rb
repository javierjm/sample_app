...
class UserTest < ActiveSupport::TestCase
  ...
  test "associated microposts should be destroyec" do 
    @user.save 
    @user.microposts.create!(content: "this is a new content")
  ...
  test "should follow and unfollow a user" do 
    javier = users(:user_1)
    laura = users(:user_2)
    assert_not javier.following?(laura)

    javier.follow(laura)
    assert javier.following?(laura)

    assert laura.followers.include?(javier)

    javier.unfollow(laura)
    assert_not javier.following?(laura)
